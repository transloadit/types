...
export const image_resize_robot_schema = z.object({
  robot: z.literal("/image/resize"),
  format: z.enum(["jpg", "png", "gif", "tiff"]).nullable().optional()
...
`),
  frame: z.union([z.number().int(), z.null()]).nullable().optional()
...
`),
  colorspace: z.enum(["sRGB", "Gray"]).optional()
...
`),
  sepia: z.number().int().max(99).nullable().optional()
...
`),
  compress: z.union([z.string(), z.null()]).nullable().optional()
...
`),
  blur: z.union([z.string(), z.null()]).nullable().optional()
...
`),
  watermark_resize_strategy: z.enum(["fit", "min_fit", "stretch", "area"]).default("fit").optional()
